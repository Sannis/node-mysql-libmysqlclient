 ...
 */
exports.createConnection = function createConnectionSync() {
  var connection = new bindings.MysqlConnection();

  var args = Array.prototype.slice.call(arguments);

  // Last arguments must be callback function
  ...
  if (args.length > 0) {
    ...
    var connectArguments = Array.prototype.slice.call(args, 0, 7);
    while (connectArguments.length < 7) {
      connectArguments.push(undefined);
    }
    ...
    // + callback
    connectArguments.push(actualCallback);

    // connection.constructor.prototype == bindings.MysqlConnection.prototype;
    bindings.MysqlConnection.prototype.connect.apply(connection, connectArguments);
  } else {
    // Run callback on next event loop tick
    // For compatibility with MysqlConnection.prototype.connect
    process.nextTick(callback.bind(null, connection));
  }
...
MysqlConnectionQueued.prototype.connect = function query() {
  ...
  // Last arguments should be callback function
  // If not, push it back
  var callback = args.pop();
  if (typeof callback != 'function') {
    args.push(callback);
